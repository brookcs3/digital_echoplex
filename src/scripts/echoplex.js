...
function initializeUIComponents() {
  ...
  if (typeof window.EchoplexAudioEngine === 'undefined') {
    console.error(
      'Error: EchoplexAudioEngine not loaded. Check script dependencies.',
    )
    showErrorMessage(
...
async function runSystemChecks(statusElement) {
  ...
  if (!isCompatible) {
    throw new Error(
      'Browser not compatible with Web Audio API. Please use Chrome, Firefox, or Edge.',
    )
  }
    ...
    windowButton.addEventListener('mouseup', () => {
      ...
      if (state.isPlaying) {
        const windowEndPosition =
          window.echoplexEngine.getCurrentPlaybackPosition()
        window.echoplexEngine.setLoopWindow(
          windowStartPosition,
          windowEndPosition,
        )
...
function startTimerUpdates() {
  ...
  if (!recordingTimeElement || !loopLengthElement || !window.echoplexEngine)
    return
