...
class EchoplexDigitalPro {
    ...
    handleButtonPress(buttonName, event) {
        ...
        // Handle sustain mode for record button
        if (buttonName === 'record' && this.state.recordMode === 'SUSTAIN') {
            if (!this.state.isRecording) {
                // Start recording immediately when button is pressed in sustain mode
                const recordLed = this.elements.recordBtn?.querySelector('.status-led');
                this.startRecording(recordLed);
                console.log('Sustain Record: Recording started on press');
            }
            return;
        }
            ...
            if (!this.state.isOverdubbing && this.state.loopTime > 0) {
                ...
                this.startOverdubRecording().then(success => {
                    if (success) {
                        ...
                        if (overdubLed) overdubLed.className = 'status-led red';
    ...
    handleButtonRelease(buttonName, event) {
        ...
        if (buttonName === 'record' && this.state.recordMode === 'SUSTAIN') {
            if (this.state.isRecording) {
                // Stop recording when button is released in sustain mode
                const recordLed = this.elements.recordBtn?.querySelector('.status-led');
                this.stopRecording();
                if (recordLed) recordLed.className = 'status-led green';
                console.log('Sustain Record: Recording stopped on release');
            }
            return;
        }
        ...
        if (buttonName === 'overdub' && this.state.overdubMode === 'SUSTAIN') {
            if (this.state.isOverdubbing) {
                ...
                this.stopOverdubRecording().then(() => {
                    ...
                    if (overdubLed) overdubLed.className = 'status-led green';
    ...
    updateButtonParameterStates() {
        ...
        buttons.forEach((buttonName, index) => {
            ...
            if (statusLed && this.state.parameterMode > 0) {
                ...
                statusLed.className = 'status-led green'; // Default state
            }
        });
    ...
    handleToggleRecord(recordLed) {
        if (this.state.isRecording) {
            // Stop recording - apply quantization if set
            if (this.state.quantizeMode !== 'OFF') {
                this.executeWithQuantization(() => {
                    this.stopRecording();
                    recordLed.className = 'status-led green';
                }, 'RECORD_STOP');
            } else {
                this.stopRecording();
                recordLed.className = 'status-led green';
            ...
            // Start recording immediately (no quantization on start)
            this.startRecording(recordLed);
        }
    }
    ...
    async continueRecordingAfterSync(recordLed) {
        ...
        try {
            ...
            recordLed.className = 'status-led red';
            ...
            // SYSTEMATIC FIX #6: Real-time display updates with accurate audio timing
            this.recordingInterval = setInterval(() => {
                // Update loop time using precise audio timing when available
        ...
        if (this.state.isOverdubbing) {
            ...
            if (this.state.quantizeMode !== 'OFF') {
                this.executeWithQuantization(async () => {
                    ...
                    this.state.isOverdubbing = false;
                    overdubLed.className = 'status-led green';
            ...
            if (this.state.quantizeMode !== 'OFF') {
                this.executeWithQuantization(async () => {
                    ...
                    this.state.isOverdubbing = true;
                    overdubLed.className = 'status-led red';
                    this.saveUndoState();
    ...
    startMultiply(multiplyLed) {
        ...
        multiplyLed.className = 'status-led red';
    ...
    stopMultiply(multiplyLed) {
        ...
        multiplyLed.className = 'status-led green';
    ...
    toggleReverse(insertLed) {
        ...
        insertLed.className = this.state.isReversed ? 'status-led red' : 'status-led green';
    ...
    toggleHalfSpeed(insertLed) {
        ...
        insertLed.className = this.state.isHalfSpeed ? 'status-led red' : 'status-led green';
    ...
    startSubstitute(insertLed) {
        ...
        insertLed.className = 'status-led red';
        this.saveUndoState();
    ...
    stopSubstitute(insertLed) {
        ...
        insertLed.className = 'status-led green';
    ...
    toggleMute() {
        ...
        if (this.state.quantizeMode !== 'OFF') {
            this.executeWithQuantization(() => {
                ...
                muteLed.className = this.state.isMuted ? 'status-led red' : 'status-led green';
    ...
    cancelCurrentInsert() {
        ...
        // Reset insert LED
        const insertLed = this.elements.insertBtn?.querySelector('.status-led');
        if (insertLed) insertLed.className = 'status-led green';
    ...
    flashUndoIndicator(type) {
        ...
        const flashInterval = setInterval(() => {
            if (step >= flashPattern.length) {
                ...
                led.className = 'status-led green'; // Return to normal
                return;
            }
    ...
    cancelMultiply() {
        ...
        // Reset multiply LED
        const multiplyLed = this.elements.multiplyBtn?.querySelector('.status-led');
        if (multiplyLed) multiplyLed.className = 'status-led green';
    ...
    updateSubstituteLED(isActive) {
        ...
        const led = insertBtn.querySelector('.status-led');
        if (led) {
            led.className = isActive ? 'status-led red' : 'status-led green';
        }
    }
            ...
            if (buttonLed) {
                switch (state) {
                    ...
                    case 'stopped':
                        ...
                        break;
                    default:
                        buttonLed.className = 'status-led green';
                }
            }
        }
    }
    ...
    createVisualTempoElements() {
        ...
        const mainInterface = document.querySelector('.main-interface');
        if (mainInterface) {
            mainInterface.appendChild(system.timingLED);
            mainInterface.appendChild(system.switchesLED);
            mainInterface.appendChild(system.multipleLEDDot);
            mainInterface.appendChild(system.tempoDot);
        }
    }
    ...
    createParameterEditingElements() {
        ...
        if (mainInterface) {
            Object.values(system.parameterLEDs).forEach(led => {
                mainInterface.appendChild(led);
            });
            mainInterface.appendChild(system.editingIndicator);
            mainInterface.appendChild(system.valueDisplay);
        }
    }
    ...
    createContinuousControllerElements() {
        ...
        const mainInterface = document.querySelector('.main-interface');
        if (mainInterface) {
            mainInterface.appendChild(system.volumeIndicator);
            mainInterface.appendChild(system.feedbackIndicator);
            mainInterface.appendChild(system.inputIndicator);
            mainInterface.appendChild(system.mixIndicator);
        }
    }
    ...
    updateLoopStateLEDs(loop) {
        ...
        if (recordLed) {
            recordLed.className = loop.isRecording ? 'status-led red' : 'status-led green';
        }
        if (overdubLed) {
            overdubLed.className = loop.isOverdubbing ? 'status-led red' : 'status-led green';
        }
        if (muteLed) {
            muteLed.className = loop.isMuted ? 'status-led red' : 'status-led green';
        }
    }
    ...
    // SYSTEMATIC FIX #6: Proper LED initialization
    initializeLEDStates() {
        const buttons = ['record', 'overdub', 'multiply', 'insert', 'mute', 'undo', 'nextloop'];
        let connectedLEDs = 0;
        ...
        buttons.forEach(buttonName => {
            ...
            if (btnElement) {
                ...
                if (statusLed) {
                    ...
                    console.log(`💡 LED initialized: ${buttonName}`);
                    connectedLEDs++;
                } else {
    ...
    endRecordWithMute() {
        ...
        const muteLed = this.elements.muteBtn?.querySelector('.status-led');
        if (muteLed) muteLed.className = 'status-led red';
    ...
    endMultiplyWithReverse() {
        ...
        this.state.isReversed = !this.state.isReversed;
        const insertLed = this.elements.insertBtn?.querySelector('.status-led');
        if (insertLed) insertLed.className = this.state.isReversed ? 'status-led red' : 'status-led green';
    ...
    // SYSTEMATIC FIX #13: Audio system integration  
    async initializeAudioSystem() {
        console.log('🎵 Initializing comprehensive audio system...');
        
        try {
            // Initialize audio context and core components
            ...
            // Initialize Tone.js integration with error handling
            const success = await this.initializeToneJS();
            
            if (!success) {
                throw new Error('Failed to initialize Tone.js');
            }
            
            console.log('✅ Echoplex Digital Pro Plus: Audio system initialized successfully');
            return true;
            
        } catch (error) {
            console.error('❌ Audio system initialization failed:', error);
            this.showDisplayMessage('Au.In', 2000);
            return false;
        }
    }
    ...
    startReMultiply() {
        ...
        const multiplyLed = this.elements.multiplyBtn?.querySelector('.status-led');
        if (multiplyLed) multiplyLed.className = 'status-led red';
    ...
    /**
     * Handle short press in toggle mode
     */
    handleShortPress(buttonName) {
        console.log(`👆 Short press: ${buttonName}`);
        
        // Route to existing button handlers
        switch (buttonName) {
            case 'record':
                this.handleRecord();
                break;
    ...
    updateLoopTimeDisplay() {
        try {
            ...
            if (display) {
                ...
                display.textContent = `${minutes}:${seconds.toString().padStart(2, '0')}.${centiseconds.toString().padStart(2, '0')}`;
            }
        } catch (error) {
        ...
        try {
            ...
            const stream = await navigator.mediaDevices.getUserMedia({ 
                audio: {
                    ...
                    sampleRate: this.audioSystem.sampleRate
                } 
            });
