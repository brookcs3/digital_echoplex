...
class EchoplexAudioEngine {
    ...
    if (this.inputGainNode && this.outputGainNode) {
      if (mode === 'ALWAYS_ON') {
        ...
        if (
          ...
          !this.state.isOverdubbing &&
          !this.state.isReplacing
        ) {
          try {
            this.inputGainNode.disconnect(this.outputGainNode)
            console.log(
              'Mic monitoring: RECORDING_ONLY - Input disconnected from output',
            )
          } catch (e) {
  ...
  connectInputForRecording() {
    if (
      this.state.micMonitoringMode === 'RECORDING_ONLY' &&
      this.inputGainNode &&
      this.outputGainNode
    ) {
      try {
        this.inputGainNode.connect(this.outputGainNode)
        console.log(
          'Mic monitoring: RECORDING_ONLY - Input connected for recording',
        )
      } catch (e) {
    ...
    if (
      ...
      if (
        ...
        try {
          this.inputGainNode.disconnect(this.outputGainNode)
          console.log(
            'Mic monitoring: RECORDING_ONLY - Input disconnected after recording',
          )
        } catch (e) {
          // Ignore errors if already disconnected
        }
...
function initializeUIComponents() {
  ...
  if (typeof EchoplexAudioEngine === 'undefined') {
    console.error(
      'Error: EchoplexAudioEngine not defined. Check script dependencies.',
    )
    showErrorMessage(
      'Script loading error. Please refresh the page and try again.',
    )
    return
  }
...
async function initializeEchoPlex(statusElement) {
  ...
  try {
    ...
    throw new Error(
      'Failed to initialize Echo Plex engine. Please refresh and try again.',
    )
  }
}
...
function setupRecordModeSelector() {
  const recordModeRadios = document.querySelectorAll(
    'input[name="record-mode"]',
  )
...
function setupMicMonitoringSelector() {
  const micMonitoringRadios = document.querySelectorAll(
    'input[name="mic-monitoring-mode"]',
  )
