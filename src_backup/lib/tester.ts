...
export class EchoplexTester {
  ...
  private async testRecordPlayback(): Promise<void> {
    ...
    try {
      ...
      this.recordTestResult(
        ...
        hasLoop,
        hasLoop
          ? 'Successfully recorded and created loop'
  ...
  private async testMultiply(): Promise<void> {
    ...
    try {
      ...
      const newLength =
        this.audioEngine.getState().loops[state.currentLoopIndex].endPoint
  ...
  private async testReverseAndHalfSpeed(): Promise<void> {
    ...
    try {
      ...
      const isReversed =
        this.audioEngine.getState().loops[state.currentLoopIndex].isReversed

      // Test half-speed
      this.audioEngine.toggleHalfSpeed()
      const isHalfSpeed =
        this.audioEngine.getState().loops[state.currentLoopIndex].isHalfSpeed
      ...
      this.recordTestResult(
        ...
        isHalfSpeed,
        isHalfSpeed
          ? 'Successfully set half-speed'
          : 'Failed to set half-speed',
      )
    } catch (error) {
      this.recordTestResult(
  ...
  private async testMute(): Promise<void> {
    ...
    try {
      ...
      const isMuted =
        this.audioEngine.getState().loops[state.currentLoopIndex].isMuted
  ...
  private async testNextPreviousLoop(): Promise<void> {
    ...
    try {
      ...
      this.recordTestResult(
        'Next/Previous Loop',
        nextWorked && prevWorked,
        nextWorked && prevWorked
          ? 'Successfully navigated loops'
          : 'Failed to navigate loops',
      )
    } catch (error) {
      this.recordTestResult(
        'Next/Previous Loop',
  ...
  private async testLoopWindowing(): Promise<void> {
    ...
    try {
      ...
      const updatedLoop =
        this.audioEngine.getState().loops[state.currentLoopIndex]
      const windowSet =
        updatedLoop.windowStart === windowStart &&
        updatedLoop.windowEnd === windowEnd
      ...
      this.recordTestResult(
        ...
        windowSet,
        windowSet
          ? 'Successfully set loop window'
          : 'Failed to set loop window',
      )
    } catch (error) {
      this.recordTestResult(
        'Loop Windowing',
  ...
  private async testMoveLoopWindow(): Promise<void> {
    ...
    try {
      ...
      const movedLoop =
        this.audioEngine.getState().loops[state.currentLoopIndex]
      const moved =
        movedLoop.windowStart !== null && movedLoop.windowStart > windowStart
  ...
  private async testSubstitute(): Promise<void> {
    ...
    try {
      ...
      const hasSubstitute =
        typeof this.advancedFunctions.startSubstitute === 'function' &&
      ...
      this.recordTestResult(
        ...
        hasSubstitute,
        hasSubstitute
          ? 'Substitute functions exist'
          : 'Substitute functions missing',
      )
    } catch (error) {
      this.recordTestResult(
        'Substitute',
  ...
  private async testSUSCommands(): Promise<void> {
    ...
    try {
      ...
      const hasSUS =
        typeof this.advancedFunctions.startSUSCommand === 'function' &&
      ...
      this.recordTestResult(
        ...
        hasSUS,
        hasSUS
          ? 'SUS command functions exist'
          : 'SUS command functions missing',
      )
    } catch (error) {
      this.recordTestResult(
        'SUS Commands',
  ...
  private async testMultiIncrease(): Promise<void> {
    ...
    try {
      ...
      const hasMultiIncrease =
        typeof this.advancedFunctions.multiIncrease === 'function'

      this.recordTestResult(
        'MultiIncrease',
        hasMultiIncrease,
        hasMultiIncrease
          ? 'MultiIncrease function exists'
          : 'MultiIncrease function missing',
      )
    } catch (error) {
      this.recordTestResult(
        'MultiIncrease',
  ...
  private async testLoopDivide(): Promise<void> {
    ...
    try {
      // Verify function exists
      const hasLoopDivide =
        typeof this.advancedFunctions.loopDivide === 'function'

      this.recordTestResult(
        'LoopDivide',
        hasLoopDivide,
        hasLoopDivide
          ? 'LoopDivide function exists'
          : 'LoopDivide function missing',
      )
    } catch (error) {
      this.recordTestResult(
        'LoopDivide',
  ...
  private async testQuantization(): Promise<void> {
    ...
    try {
      ...
      const loopQuantize =
        this.audioEngine.getState().settings.quantize === 'LOOP'

      this.audioEngine.updateSettings({ quantize: '8TH' })
      const eighthQuantize =
        this.audioEngine.getState().settings.quantize === '8TH'

      this.audioEngine.updateSettings({ quantize: 'CYC' })
      const cycleQuantize =
        this.audioEngine.getState().settings.quantize === 'CYC'
      ...
      this.recordTestResult(
        ...
        quantizeWorks,
        quantizeWorks
          ? 'Quantization features work correctly'
          : 'Issues with quantization features',
      )
    } catch (error) {
      this.recordTestResult(
        'Quantization',
  ...
  private async testSyncFeatures(): Promise<void> {
    ...
    try {
      ...
      const hasSync =
        typeof this.advancedFunctions.syncToExternalClock === 'function'
      ...
      this.recordTestResult(
        ...
        syncFeaturesExist
          ? 'Sync features exist'
          : 'Some sync features are missing',
      )
    } catch (error) {
      this.recordTestResult(
        'Sync Features',
  ...
  private async testInterfaceModes(): Promise<void> {
    ...
    try {
      ...
      for (const mode of modes) {
        ...
        const modeSet =
          this.audioEngine.getState().settings.interfaceMode === mode
        if (!modeSet) {
          allModesWork = false
          break
        }
      ...
      this.recordTestResult(
        ...
        allModesWork,
        allModesWork
          ? 'All interface modes work correctly'
          : 'Issues with interface modes',
      )
    } catch (error) {
      this.recordTestResult(
        'Interface Modes',
  ...
  private async testSamplerStyles(): Promise<void> {
    ...
    try {
      ...
      for (const style of styles) {
        ...
        const styleSet =
          this.audioEngine.getState().settings.samplerStyle === style
        if (!styleSet) {
          allStylesWork = false
          break
        }
      ...
      this.recordTestResult(
        ...
        allStylesWork,
        allStylesWork
          ? 'All sampler styles work correctly'
          : 'Issues with sampler styles',
      )
    } catch (error) {
      this.recordTestResult(
        'Sampler Styles',
  ...
  private async testPresetSaveLoad(): Promise<void> {
    ...
    try {
      ...
      const settingsRestored =
        state.settings.quantize === testSettings.quantize &&
  ...
  private async testSessionSaveLoad(): Promise<void> {
    ...
    try {
      ...
      const hasSessionFunctions =
        typeof this.presetManager.saveSession === 'function' &&
  ...
  private async testAudioExportImport(): Promise<void> {
    ...
    try {
      // Verify functions exist
      const hasExportImport =
        typeof this.presetManager.exportAudio === 'function' &&
      ...
      this.recordTestResult(
        ...
        hasExportImport,
        hasExportImport
          ? 'Audio export/import functions exist'
          : 'Audio export/import functions missing',
      )
    } catch (error) {
      this.recordTestResult(
        'Audio Export/Import',
  ...
  private reportResults(): void {
    ...
    for (const result of this.testResults) {
      console.log(
        `${result.passed ? '✅' : '❌'} ${result.feature}: ${result.notes}`,
      )
